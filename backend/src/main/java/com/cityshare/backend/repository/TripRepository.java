package com.cityshare.backend.repository;
...
public interface TripRepository extends JpaRepository<Trip, Long> {
    ...
    @Query("SELECT t FROM Trip t WHERE " +
           ...
           "t.status = com.cityshare.backend.entity.Trip.TripStatus.PENDING")
    List<Trip> searchTrips(@Param("dep") String departure,
                           @Param("arr") String arrival,
                           @Param("from") LocalDateTime from,
                           @Param("seats") int seats);

    @Query("SELECT t FROM Trip t WHERE t.status = com.cityshare.backend.entity.Trip.TripStatus.PENDING AND t.departureTime >= :now ORDER BY t.departureTime ASC")
